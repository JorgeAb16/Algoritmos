class Array(object):
    ...
    def bubbleSort2(self):
     ...
     while left < right:
        # Mover el elemento más grande hacia el final (lado derecho)
        for i in range(left, right):
            if self.__a[i] > self.__a[i + 1]:
                self.swap(i, i + 1)
        right -= 1  # Reducir la sección no ordenada desde la derecha

        # Mover el elemento más pequeño hacia el inicio (lado izquierdo)
        for i in range(right, left, -1):
            if self.__a[i] < self.__a[i - 1]:
                self.swap(i, i - 1)
        left += 1  # Reducir la sección no ordenada desde la izquierda
    ...
    def insertionSort(self):  # Sort by repeated inserts
        for outer in range(1, self.__nItems):  # Mark one element
            ...
            while inner > 0 and temp < self.__a[inner-1]:  # If marked elem smaller,
                self.__a[inner] = self.__a[inner-1]  # shift elem to right
                inner -= 1
